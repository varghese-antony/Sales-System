...
export const getProductPriceSummaryPerUnit = (product: any, quantity: number) => {
...
export const getPriceOptimizationSuggestions = (cartItems: any[]) => {
    const suggestions = []
    ...
    const currentTotals = {
        '20ft': { total: 0, space: 0, hasAdminFee: false },
        '40ft': { total: 0, space: 0, hasAdminFee: false }
    }
    ...
    validItems.forEach(item => {
        ...
        if (summary['20ft'] && Object.keys(summary['20ft']).length > 0) {
            currentTotals['20ft'].total += summary['20ft'].product_price + summary['20ft'].tarrif + summary['20ft'].shipment_cost + summary['20ft'].admin_consolidation_fee
            currentTotals['20ft'].space += summary['20ft'].space_occupied_cubic_meters
            if (summary['20ft'].admin_consolidation_fee > 0) {
                currentTotals['20ft'].hasAdminFee = true
            }
        }
        if (summary['40ft'] && Object.keys(summary['40ft']).length > 0) {
            currentTotals['40ft'].total += summary['40ft'].product_price + summary['40ft'].tarrif + summary['40ft'].shipment_cost + summary['40ft'].admin_consolidation_fee
            currentTotals['40ft'].space += summary['40ft'].space_occupied_cubic_meters
            if (summary['40ft'].admin_consolidation_fee > 0) {
                currentTotals['40ft'].hasAdminFee = true
            }
        }
    })
    ...
    for (const containerType of ['20ft', '40ft'] as const) {
    ...
    const current20ftTotal = currentTotals['20ft'].total
    const current40ftTotal = currentTotals['40ft'].total
    ...
    if (current40ftTotal > 0 && current20ftTotal > 0) {
        ...
        if (current40ftTotal < current20ftTotal) {
            const container20ft = SHIPMENT_CONTAINER_DETAILS.find(c => c.container_type === '20ft')
            if (container20ft) {
                const maxSpace20ft = container20ft.container_size_in_cubic_meters
                const currentSpace40ft = currentTotals['40ft'].space
                ...
                if (currentSpace40ft > maxSpace20ft) {
                    ...
                    validItems.forEach(item => {
                        const priceData = getProductPriceSummary(item, 1)
                        if (priceData['40ft'] && Object.keys(priceData['40ft']).length > 0) {
                            const costPerCubicMeter = (priceData['40ft'].product_price + priceData['40ft'].tarrif + priceData['40ft'].shipment_cost) / item.cubic_m_per_pc
                    ...
                    if (bestProductToReduce) {
                        ...
                        if (quantityToRemove > 0 && quantityToRemove < bestProductToReduce.quantity) {
                            ...
                            const newTotal = calculateTotalForQuantities(newItems, '20ft')
