...
export default function IndoorProductPage({ params }) {
  ...
  const keyDescriptions = {
    ...
    'Power (W)': 'Select the power consumption that meets your efficiency needs',
    'Voltage': 'Pick the voltage compatible with your electrical system',
    'CCT': 'Choose the color temperature for the desired ambiance',
    'CRI/RA': 'Select the color rendering index for accurate color representation',
    'Lumen': 'Pick the brightness level suitable for your application',
    'Efficacy (lm/W)': 'Choose the energy efficiency rating',
  ...
  const goBack = () => {
    ...
    if (currentStep > 0) {
      ...
      if (Object.keys(newFilters).length === 0) {
        ...
        fetchData(`https://n8n.werposolutions.com/webhook/get-model?table=indoor&query=${encodeURIComponent(query)}`)
          .then(setProducts)
          .catch(console.error)
      }
    }
