...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Demo mode banner — shown across top of page when active */}
      {demoMode && (
        <div style={{
          position:'fixed', top:0, left:220, right:0, zIndex:200,
          background:'rgba(0,246,255,0.07)', borderBottom:'1px solid rgba(0,246,255,0.15)',
          padding:'6px 20px', display:'flex', alignItems:'center', justifyContent:'space-between',
        }}>
          <span style={{ fontSize:12, color:'rgba(0,246,255,0.7)', fontWeight:500 }}>
            🔒 Demo mode on — sensitive data is blurred
          </span>
          <button onClick={toggleDemoMode} style={{
            fontSize:11, color:'rgba(0,246,255,0.5)', background:'none', border:'none',
      ...
      <aside style={{
        ...
        <div style={{ padding:'0 8px 8px' }}>
          <button onClick={toggleDemoMode} style={{
            ...
            background: demoMode ? 'rgba(0,246,255,0.1)' : 'rgba(255,255,255,0.03)',
            transition:'all 0.2s',
          }}>
            {/* pill toggle */}
            <div style={{
              width:28, height:16, borderRadius:99, flexShrink:0, transition:'all 0.2s',
              background: demoMode ? '#00F6FF' : 'rgba(255,255,255,0.1)',
              position:'relative',
            }}>
              <div style={{
                position:'absolute', top:2, left: demoMode ? 14 : 2,
                width:12, height:12, borderRadius:'50%',
                background: demoMode ? '#080810' : 'rgba(255,255,255,0.4)',
                transition:'left 0.2s',
              }}/>
            </div>
            <span style={{
              fontSize:12, fontWeight:600,
              color: demoMode ? '#00F6FF' : '#4A4F6A',
              transition:'color 0.2s',
            }}>
              Demo Mode
            </span>
            {demoMode && (
              <span style={{ marginLeft:'auto', fontSize:9, color:'rgba(0,246,255,0.5)', fontWeight:600 }}>ON</span>
            )}
          </button>
        </div>

        {/* User */}
        <div style={{ padding:'8px 12px 16px', borderTop:'1px solid rgba(0,246,255,0.06)' }}>
